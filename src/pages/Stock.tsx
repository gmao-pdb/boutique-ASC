...
export default function Stock() {
  ...
  return (
    <>
      ...
      {vue === "articles" && (
        <>
          <label className="check"><input type="checkbox" checked={alertesSeules} onChange={(e) => setAlertesSeules(e.target.checked)} /> Afficher seulement les alertes</label>
          <button className="mini" style={{ margin: "10px 0" }} onClick={() => void faireInventaire()}>🗒️ Enregistrer l'inventaire</button>

          {cfg.catalogue.map((art) => {
            const rows = art.tailles.map((t) => {
              const s = stockMap.get(stockId(art.nom, t));
              const q = s?.quantite ?? 0, seuil = s?.seuilMini ?? 0;
              return { t, q, seuil, rupture: q <= 0, bas: q > 0 && q <= seuil };
            });
            const shown = alertesSeules ? rows.filter((r) => r.rupture || r.bas) : rows;
            if (alertesSeules && shown.length === 0) return null;
            const alerte = rows.some((r) => r.rupture) ? " 🔴" : rows.some((r) => r.bas) ? " 🟠" : "";
            return (
              <details key={art.nom} className="stock-cat">
                <summary>{art.nom}{alerte}</summary>
                {shown.map((r) => (
                  <div key={r.t} className="stock-row">
                    ...
                    <input type="number" value={r.seuil} onChange={(e) => void setStockItem(art.nom, r.t, { seuilMini: Math.max(0, Math.round(+e.target.value || 0)) })} />
                    {r.rupture ? <span className="badge no">rupt.</span> : r.bas ? <span className="badge part">bas</span> : <span className="badge ok">ok</span>}
                    <button className="x" onClick={() => supprimerTaille(art.nom, r.t)}>✕</button>
                  </div>
                ))}
                {!alertesSeules && (
                  <div className="addrow">
                    <input placeholder="ajouter une taille (ex. M)" value={newSize[art.nom] || ""} onChange={(e) => setNewSize({ ...newSize, [art.nom]: e.target.value })} onKeyDown={(e) => { if (e.key === "Enter") ajouterTaille(art.nom); }} />
                    <button className="mini" onClick={() => ajouterTaille(art.nom)}>+ taille</button>
                    <button className="x" onClick={() => supprimerArticle(art.nom)} title="Supprimer l'article">🗑️</button>
                  </div>
                )}
              </details>
            );
